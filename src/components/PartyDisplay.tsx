...
export const PartyDisplay: React.FC<PartyDisplayProps> = ({ 
  ...
  // Development-only function to "kill" a party member by dealing damage equal to their current HP
  const killPartyMember = (memberIndex: number) => {
    if (!isDev() || !setParty) return;
    
    setParty(prevParty => {
      return prevParty.map((member, index) => {
        if (index !== memberIndex) return member;
        
        // Deal damage equal to current HP to simulate a lethal hit
        const damage = member.hp;
      ...
      <div className="bg-slate-800/60 backdrop-blur-sm rounded-xl p-4 border border-slate-700">
        ...
        <div className="space-y-3">
          {party.map((member, index) => {
            const IconComponent = member.icon || (() => <div>?</div>);
            // Validate HP values to prevent NaN display
            const safeHp = typeof member.hp === 'number' && !isNaN(member.hp) ? member.hp : 0;
            const safeMaxHp = typeof member.maxHp === 'number' && !isNaN(member.maxHp) && member.maxHp > 0 ? member.maxHp : 100;
            const hpPercent = (safeHp / safeMaxHp) * 100;
            const isLowHp = hpPercent <= 30;
            const isDead = safeHp <= 0;
              ...
              <div key={index} className={`rounded-lg p-3 border transition-all ${
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-1 rounded text-white font-semibold bg-${classColor}-500/30`}>
                        {(member.role || 'UNKNOWN').toUpperCase()}
                      </span>
                      {/* Development-only kill button */}
                      {isDev() && setParty && member.hp > 0 && (
                        <button
                          onClick={() => killPartyMember(index)}
                          className="text-xs px-2 py-1 rounded bg-red-600/30 text-red-300 hover:bg-red-600/50 transition-colors font-semibold"
                          title={`[DEV] Kill ${member.name} for testing`}
                        >
                ...
                <div className="bg-slate-600/50 rounded-lg p-2 border border-slate-500/30">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-purple-300 font-semibold truncate">{member.skill?.name || 'Unknown Skill'}</span>
                ...
                {/* Attack Timer Display - Always visible to prevent UI shifts */}
                <div className="mt-2">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-slate-400">Next Attack</span>
                    <span className={`text-xs px-1 rounded ${
                      !gameState.inCombat ? 'bg-slate-600/30 text-slate-500' :
                      member.hp <= 0 ? 'bg-red-600/30 text-red-400' :
                  ...
                  <div className="w-full bg-slate-700/50 rounded-full h-1.5 border border-slate-600/30">
                    <div 
                      className={`h-1.5 rounded-full transition-all ease-linear ${
                        !gameState.inCombat ? 'bg-gradient-to-r from-slate-500 to-slate-400' :
                        member.hp <= 0 ? 'bg-gradient-to-r from-red-500 to-red-400' :
                      ...
                      style={{ 
                        width: `${!gameState.inCombat || member.hp <= 0 ? 0 : member.attackTimer || 0}%`,
